...
def prepare_to_answer(request, users_to_return, user_roles, user):
    """Execure parsing the request and prepare for compuring the data"""
...
def get_all_users(collection):
    """Returns the list of unique users"""
...
def get_users_list(collection, username, from_date, end_date):
    """Returns the list of users that were met by a specific user"""
    db = db_wrapper.DatabaseHelper()
...
def _get_last_scan_id(collection):
    ...
    db = db_wrapper.DatabaseHelper()
    last_scan = db.execute_named_query(NAMED_QUERIES['get_last_scan_id_' + collection], ()).fetchone()['MAX(scan_id)']
    if last_scan == None:
        return 0
    return last_scan
    
def _compute_who_did_i_see(collection, scan_id):
    """Performs computations for the run method of this question"""
    while True:
        ...
        _update_question_db(collection, chunk_dict, last_scan_id)
        #_update_last_scan_id(collection, last_scan_id)
        
def _get_new_scan_chunk(collection, scan_id, SCAN_CHUNK):
    """Retrieve the scan chunk that has to be processed"""
    db = db_wrapper.DatabaseHelper()
...
def _perform_computations(scan_list):
    """Compute the meeting time for all the users met"""
...
def _update_question_db(collection, chunk_dict, last_scan_id):
    """Update the database with question data"""
    db = db_wrapper.DatabaseHelper()
    update_rows = []
        ...
        for user in chunk_dict[timestamp]:
            for user_seen in chunk_dict[timestamp][user]:
                ...
                if time_spent >= MIN_TIME or _meeting_occured(collection, timestamp, user, user_seen):
                    update_rows.append({'timestamp' : timestamp, 'user': user, 'user_seen': user_seen, 'time': time_spent, 'scan_id': last_scan_id})
    #commit
    db.insert_rows(update_rows, "question_who_did_i_see", [collection])
    return

def _meeting_occured(collection, hour, user, user_seen):
    """Filters the data from < 5 minutes long meetings"""
